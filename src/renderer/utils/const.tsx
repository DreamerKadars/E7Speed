...
export const E7DataDomain = '/Data';
...
export const MainNecklaceRange = [
  ClassCC,
  ClassCD,
  ClassAtkPercent,
  ClassDefendPercent,
...
// 计算高速装备的阀值
export function GetHighSpeedThreshold(
  ...
  if (
    (EquipLoc === EquipLocRing || EquipLoc === EquipLocNecklace) &&
    (MainClassType === ClassAtk ||
      MainClassType === ClassDefend ||
      MainClassType === ClassHp)
  ) {
...
export const CalEquipSubValue = (equip: Equipment) => {
  ...
  if (
    equip.FourLocClass !== '' &&
    equip.UpgradeLevel >= 12 &&
    equip.EquipColor === 'purple'
  ) {
    let newSubValues = [
      { Class: '', Value: 0 },
      { Class: '', Value: 0 },
...
export function GetSubValueClassFourRange(equip: Equipment) {
  ...
  let res = ConvertStringToSubValue(
    range.filter((classType) => {
...
export function GetClassTransform(
  subClass: string,
  timeUpgrade: number,
  equipLevel: number,
) {
  ...
  if (equipLevel === 90 || equipLevel === 85) {
    // 85当作可以重铸的90来计算
    switch (subClass) {
      case ClassAtk:
...
export function GetOneValueDistributionMapKey(
  classType: string,
  level: number,
  intensifyNum: number,
) {
...
export const CalGradeByClass = (
  heroInfo: CalculatedStatus | undefined,
  classType: string,
  value: number,
) => {
  switch (classType) {
    case ClassAtk:
      ...
      return Number(
        (100 * (value / heroInfo.lv60SixStarFullyAwakened[FKeyAtk])).toFixed(2),
      );
    case ClassDefend:
      if (heroInfo === undefined) {
        return value / 6;
      ...
      return Number(
        (100 * (value / heroInfo.lv60SixStarFullyAwakened[FKeyDef])).toFixed(2),
      );
    case ClassHp:
      if (heroInfo === undefined) {
        return value / 50;
      ...
      return Number(
        (100 * (value / heroInfo.lv60SixStarFullyAwakened[FKeyHP])).toFixed(2),
      );
    default:
...
export const CalMainValueByClass = (
  heroInfo: CalculatedStatus,
  mainClassType: string,
  classType: string,
) => {
...
export const CalMainGradeByClass = (
  heroInfo: CalculatedStatus,
  mainClassType: string,
) => {
  let mainValue = ClassMainMap.get(mainClassType);
...
export const CalClassSetValue = (
  heroInfo: CalculatedStatus | undefined,
  setTypes: string[],
  needClassType: string,
) => {
  if (heroInfo === undefined) {
    return 0;
  }
...
export function GetHeroDetailFromMap(
  heroCode: string,
  HeroListMap: Map<string, HeroDetail>,
) {
...
export function GetGradeAimStrategiesByLocal() {
  ...
  res.map((_, index) => {
    let storageGradeStr = localStorage.getItem(
      res[index].Description + '#Grade',
    );
...
export interface HeroTemplatePreComputationForAdapter {
  HeroCode: string[]; // 该需求的英雄代码
  HeroTemplateID: string[]; // 该需求的模板代码
  Tag: string[];
  // AtkBase: number,
  // DefendBase: number,
  // HpBase: number,
  SubValueNeed: string[]; // 该需求的需要子属性
  GradeThreshold: number; // 分数阈值
  NeedClassTypeNum: number; // 在该装备上的属性需求数量
...
function IsMainClassExist(
  heroTemplate: HeroTemplate,
  EquipLoc: string,
  MainType: string,
) {
  let res = false;
  if (IsRightLoc(EquipLoc)) {
    // 右三主属性匹配
    let rangeMainClass: string[] = [];
    switch (EquipLoc) {
      case EquipLocNecklace:
        rangeMainClass =
          heroTemplate.MainNecklace === undefined
            ? []
            : heroTemplate.MainNecklace;
        break;
      case EquipLocRing:
        rangeMainClass =
          heroTemplate.MainRing === undefined ? [] : heroTemplate.MainRing;
        break;
      case EquipLocShoes:
        rangeMainClass =
          heroTemplate.MainShoes === undefined ? [] : heroTemplate.MainShoes;
        break;
    }
    rangeMainClass.map((classType: string) => {
      if (classType === MainType) {
        res = true;
      }
    });
    return res;
...
function TemplateSetExist(heroTemplate: HeroTemplate, Set: string) {
  // 套装匹配
...
function CalConvertValueType(equip: Equipment, subValueNeed: OneEquipConfInfo) {
  ...
  CanConvertList.map((subValue) => {
    ...
    if (need === undefined) {
      ...
      if (level > 0) {
        if (
          (subValue.Class === ClassAtk ||
            subValue.Class === ClassHp ||
            subValue.Class === ClassDefend) &&
...
function CalNeedClassNumInTemplate(
  equip: Equipment,
  subValueNeed: OneEquipConfInfo,
) {
  let needClassNumInTemplate = 0;
  let subValueRange = GetSubValueRange(equip.EquipLoc, equip.MainType);
  let a = subValueRange.map((subValueClass) => {
    let need: CalculateClassGradeNeedOneRes | undefined;
  ...
  return needClassNumInTemplate;
}

function CalNeedClassNumInEquip(
  subValueNeed: OneEquipConfInfo,
  SubValues: SubValue[],
) {
  ...
  return {tempSubValueNeed,needClassNumInEquip}
}

// 需要的属性、阀值、转换属性
export function CalHeroTemplatePreComputationForAdapter(
  equip: Equipment,
  SubValues: SubValue[],
  ...
  HeroTemplateList.map((heroTemplate: HeroTemplate) => {
    if (!CheckHeroTemplate(HeroDetailMap, heroTemplate)) {
      return;
    }
    let heroInfo = HeroDetailMap.get(heroTemplate.HeroCode!);
    if (!TemplateSetExist(heroTemplate, equip.Set!)) {
      // 套装存在
      return;
    }
    if (!IsMainClassExist(heroTemplate, equip.EquipLoc, equip.MainType)) {
      // 主属性匹配
      return;
    }
    ...
    let convertValueType = CalConvertValueType(equip, subValueNeed); // 算一下把没有的属性转掉有多少转换收益

    let subValueRange = GetSubValueRange(equip.EquipLoc, equip.MainType);
    // 计算这个模板在这个部位需要多少种属性，需要的属性数量决定目标
    let needClassNumInTemplate = CalNeedClassNumInTemplate(equip, subValueNeed);

    // 需要的副属性进行筛选和分级,然后通过需求度分级
    let resNeedClass=CalNeedClassNumInEquip(subValueNeed,SubValues)
    let preComputation: HeroTemplatePreComputationForAdapter;

    if (resNeedClass.needClassNumInEquip < 2) {
      // 可利用的属性只有一个，那么不适合参与计算
      return;
    }
    ...
    if (
      equip.EquipLoc === EquipLocNecklace ||
      equip.EquipLoc === EquipLocRing ||
      equip.EquipLoc === EquipLocShoes
    ) {
    ...
    GradeAimStrategies.map((GradeAimStrategy) => {
      if (
        GradeAimStrategy.Loc === locLeftOrRight &&
        GradeAimStrategy.NeedClassNum === needClassNumInTemplate
      ) {
    ...
    let needCode = '';
    resNeedClass.tempSubValueNeed.map((needTemp) => {
      if (needTemp!=='') {
        needCode += '1';
      } else {
        needCode += '0';
    ...
    if (tempMap.get(needCode) === undefined) {
      preComputation = {
        ...
        SubValueNeed: resNeedClass.tempSubValueNeed,
...
export function GetSelfDevotionGradeValueByLevel(
  level: string,
...
export function CreateHeroPanelWithUP(heroDetailBackEnd?: HeroDetailBackEnd) {
  ...
  if (
    heroDetailBackEnd !== undefined &&
    heroDetailBackEnd.calculatedStatus !== undefined
  ) {
    temp.ATK.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyAtk];
    temp.DF.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyDef];
    temp.SP.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeySpeed];
    temp.HP.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyHP];
    temp.CC.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyCC] * 100;
    temp.CD.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyCD] * 100;
    temp.HR.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyHR] * 100;
    temp.RR.Down =
      heroDetailBackEnd.calculatedStatus.lv60SixStarFullyAwakened[FKeyRR] * 100;
  }
  return temp;
}
...
export const IntensifyDistributions: IntensifyDistribution[] = [
  ...
  },
  {
    EquipType: 'red',
    StartLevel: 15,
    Distribution: new Map<string, number>([['0000', 1]]),
    Total: 1,
  },
  {
    EquipType: 'purple',
...
export interface ExtraValueShowInfo {
  reason: string;
  value: number;
}
...
export function CalculateClassGradeNeedOne(req: CalculateClassGradeNeedOneReq) {
  ...
  if (aim === undefined || aim.Enable === false) {
    ...
    mainValue =
      CalMainValueByClass(heroInfo, mainNecklace, classType) +
      CalMainValueByClass(heroInfo, mainRing, classType) +
      CalMainValueByClass(heroInfo, mainShoes, classType);
...
export function GetArtifactValueByLevel(
  artifactCode: string,
  level: number,
  allArtifactInfoList: ArtifactListResult,
) {
  ...
  allArtifactInfoList.artifactList.map(
    (artifactTemp: ArtifactInfo, artifactIndex: number) => {
  ...
  return {
    HP: Number(
      (
        (tempArtifactInfo.abilityDefense * (30 - level) +
          tempArtifactInfo.enhanceAbilityDefense * level) /
        30
      ).toFixed(2),
    ),
    ATK: Number(
      (
        (tempArtifactInfo.abilityAttack * (30 - level) +
          tempArtifactInfo.enhanceAbilityAttack * level) /
        30
      ).toFixed(2),
    ),
  };
...
export function CalculateOneEquipConfInfoByTemplate(
  req: CalculateOneEquipConfInfoByTemplateReq,
) {
  const {
    template,
    heroDetailBackEnd,
  ...
  if (
    heroDetailBackEnd.extra_panels !== undefined &&
    heroDetailBackEnd.extra_panels !== null
  ) {
  ...
  if (
    ...
    artifactInfo !== null &&
    artifactInfo[0] !== undefined &&
    allArtifactInfoList !== undefined
  ) {
    let res = GetArtifactValueByLevel(
      artifactInfo[0].ArtifactCode === undefined
        ? ''
        : artifactInfo[0].ArtifactCode,
  ...
  if (selfDevotion !== undefined && selfDevotion !== '') {
    let value = GetSelfDevotionGradeValueByLevel(
      selfDevotion,
      heroDetailBackEnd.self_devotion,
    );
    extra_panels.push({
      reason: selfDevotion + '自阵加成',
      effectValue: [
        {
          classType: ConvertSelfDevotionTypeToCommon(
            heroDetailBackEnd.self_devotion?.type,
          ),
          value: value,
        },
      ],
  ...
  // 计算专属装备的加成加到额外配置上
  if (
    heroDetailBackEnd.eeType !== undefined &&
    heroDetailBackEnd.eeType !== ''
  ) {
    extra_panels.push({
      reason: '专属装备加成',
      effectValue: [
...
// 猜测强化次数，只计算85装备
export function GuessUpgradeDistribution(
  subValues: SubValue[],
  upgradeTime: number,
  equipColor: string,
) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    subValues.map((subValue, index) => {
      ...
      let nextUpgradeTimeProbability = CalSubValueNumberProbability(
        subValue,
        index,
        nextUpgradeTime,
        upgradeTime,
...
function CalSubValueNumberProbability(
  subValue: SubValue,
  subValueIndex: number,
  theValueUpgradeTime: number,
  allUpgradeTime: number,
  equipColor: string,
) {
  // 条件概率 P(A|B)=P(AB)/P(B)
  // A:强化次数为upgradeTime
  ...
  let intensifyDistribution: IntensifyDistribution =
    new IntensifyDistribution();
  let findFlag = false;
  IntensifyDistributions.map((intensifyDistributionTemp) => {
    if (
      intensifyDistributionTemp.EquipType === equipColor &&
      intensifyDistributionTemp.StartLevel === 15 - allUpgradeTime * 3
    ) {
  ...
  for (let i = 0; i <= allUpgradeTime + 1; i++) {
    ...
    for (let key of intensifyDistribution.Distribution.keys()) {
      // 遍历所有的强化次数分配
      // 当前分布的比重
      let value = intensifyDistribution.Distribution.get(key)!;
      // 保证key的长度为4，计算出每个属性的强化次数
      let intensifyNumDistribution = [
        Number(key[0]) - Number('0'),
        Number(key[1]) - Number('0'),
        Number(key[2]) - Number('0'),
        Number(key[3]) - Number('0'),
      ];
  ...
  for (let i = 0; i <= allUpgradeTime + 1; i++) {
    ...
    P_Value_OnUpgradeTime.push(
      CalValueProbabilityOnUpgradeTime(range, i, subValue.Value),
    );
  }
  ...
  let P_AB =
    P_UpgradeTime[theValueUpgradeTime] *
    P_Value_OnUpgradeTime[theValueUpgradeTime];
  let P_B = 0;
  for (let i = 0; i <= allUpgradeTime + 1; i++) {
...
function CalValueProbabilityOnUpgradeTime(
  range: number[],
  upgradeTime: number,
  value: number,
) {
...
function CalValueDistributionOnUpgradeTime(
  range: number[],
  upgradeTime: number,
) {
  let valueTop = 100;
